...
public class KingRule implements ChessRuleMove {
    BoardManager bm;
    Board board;
    ...
    public KingRule(BoardManager bm, Board board, int oldX, int oldY, int newX, int newY){
        this.bm = bm;
        this.board = board;
        positionOld[0] = oldX;
        positionOld[1] = oldY;
    ...
    public boolean move() {
        ...
        return Math.abs(positionOld[0] - positionNew[0]) <= 1 &&
                Math.abs(positionOld[1] - positionNew[1]) <= 1;
        // check piece rules.
    }
