...
public class RookRule implements ChessRuleMove {
    BoardManager bm;
    Board board;
    ...
    public RookRule(BoardManager bm, Board board, int oldX, int oldY, int newX, int newY){
        this.bm = bm;
        this.board = board;
        positionOld[0] = oldX;
        positionOld[1] = oldY;
    ...
    public boolean move() {
        ...
        // check basic rules.
        return Math.abs(positionOld[0] - positionNew[0]) == 0 || Math.abs(positionOld[1] - positionNew[1]) == 0;
        // check piece rules.
    }
